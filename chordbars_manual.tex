...
Thus, to be able to have the same bar count on the software and on the sheet produced with this software, the command {\tt \textbackslash hasCountIn} can be used so that the bar count does not start at 1, but rather at 2 or 3, to include the count-in.

The default Count In value is 1 bar, but you can select 1 or 2 by issuing either
{\tt \textbackslash CountInOneBar}
or
{\tt \textbackslash CountInTwoBars}
before {\tt \textbackslash hasCountIn}.
